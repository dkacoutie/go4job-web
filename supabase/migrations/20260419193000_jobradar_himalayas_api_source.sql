...
where not exists (
  ...
  where code = 'himalayas_api'
);

update public.job_sources
set
  name = 'Himalayas API',
  ingest_method = 'api_himalayas',
  ingest_status = 'ready',
...
begin
  if exists (
    ...
    execute $sql$
      update public.job_sources
      set base_url = 'https://himalayas.app/jobs'
      where code = 'himalayas_api'
    $sql$;
  end if;
  ...
  if exists (
    ...
    execute $sql$
      update public.job_sources
      set url = 'https://himalayas.app/jobs'
      where code = 'himalayas_api'
    $sql$;
  end if;
  ...
  if exists (
    ...
    execute $sql$
      update public.job_sources
      set notes = 'Staging only. Himalayas JSON API. Small subset preview. Respect 24h refresh and Himalayas attribution.'
      where code = 'himalayas_api'
    $sql$;
  end if;
