// Morocco Emploi.ma pilot: dry-run only from ingest_source/index.ts.

export type EmploiMaPortalItem = {
  external_id: string;
  title: string;
...
const FIRST_PAGE_URL = `${BASE_URL}/recherche-jobs-maroc`;
...
async function buildExternalId(item: {
  ...
  if (numericId) return `emploi_ma_portal:${numericId}`;
  const hash = await sha256Hex(
    `${item.title}|${item.company_name ?? ""}|${item.published_at ?? ""}|${item.source_url}`,
  );
  return `emploi_ma_portal:${hash}`;
}

function extractMaxPages(html: string) {
  let max = 1;
  const pageRe = /recherche-jobs-maroc\?page=(\d+)/gi;
...
async function parseOffersFromHtml(html: string) {
  ...
  const items: EmploiMaPortalItem[] = [];
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    if (!sourceUrl || !sourceUrl.includes("/offre-emploi-maroc/")) {
      continue;
    }
    ...
    const suspiciousTerms = detectSuspiciousTerms({
      title,
...
async function fetchPage(url: string) {
  ...
  try {
    ...
    if (elapsedMs > PAGE_TIMEOUT_MS) {
      throw new Error(`emploi_ma_portal page fetch exceeded ${PAGE_TIMEOUT_MS}ms`);
    }
    if (!res.ok) {
      throw new Error(`emploi_ma_portal list fetch failed: ${res.status} ${res.statusText}`);
    }
...
export async function fetchEmploiMaPortalItems(
  limit = 30,
